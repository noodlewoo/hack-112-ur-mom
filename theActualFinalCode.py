...
def redrawAll(app):
    ...
    if app.gameOver and not app.actuallyGameOver:
        ...
        if app.succeed == True:
            ...
            drawLabel(f"Good job! You collected {app.foodCounter} food.",210, 70, size=12, font = "monospace", bold=True, fill = 'white')
            drawLabel("You actually learned something! ", 210, 90, size=12, font = "monospace", bold=True, fill = 'white')
        else:
            drawLabel(f"Game Over. You collected {app.foodCounter} food.",210, 70, size=12, font = "monospace", bold=True, fill = 'white')
            drawLabel("(study skills gained??)", 210, 90, size=12, font = "monospace", bold=True, fill = 'white')
        
    if app.studyGame and not app.gameOver:
        drawRect(30, 60, 360, 230, fill='black')
        drawLabel("OMG! you're Mike's axolotl! :0", 230, 70, size=12, font = "monospace", bold=True, fill = 'white')
        drawLabel('Collect all of the skittles! use the arrow keys',210,85,size=12, font = "monospace", bold=True, fill = 'white')
        drawLabel(f"amount eaten: {app.foodCounter}, time left: {30 - app.gameTime}",210,100,fill = 'white')
        x,y = app.kimchee
